...
cparams.eps = 1.0e-6;
cparams.nover = 0;
cparams.maxchunklen = min(0.2, 4./zk); % setting a chunk length helps when the
                              % frequency is known
...
amp = 0.25;
scale = 2;
start = tic; chnkr = chunkerfunc(@(t) starfish(t,narms,amp,[], [], scale),cparams,pref); 
t1 = toc(start);
...
opts = [];
opts.sing = 'log';
fkern =  @(s,t) chnk.flex2d.kern(zk, s, t, 'free plate first part interior', coefs);        % build the desired kernel
...
fkern3 = @(s,t) chnk.flex2d.kern(zk, s, t, 'free plate K21 first part interior', coefs);                     % singularity subtration kernel in K21 (including swapping its Asmyptotics expansions)

fkern4 = @(s,t) chnk.flex2d.kern(zk, s, t, 'free plate K21 second part interior', coefs);                    % kernels in K21 needs to multiply by curvature
...
[~, ~, hess, third, ~] = chnk.flex2d.helmdiffgreen(zk, [5;5], chnkr.r);
[~, ~, hessK, thirdK, ~] = chnk.flex2d.helmdiffgreen(zk*(1i), [5;5], chnkr.r);
...
xs = -3:0.05:3;                                     % generate some targets
ys = -3:0.05:3;
[X,Y] = meshgrid(xs, ys);
targets = [X(:).'; Y(:).'];
...
coupled = chunkerkerneval(chnkr, ikern3, H*rho1, targets(:, in));

start1 = tic;
Dsol = -chunkerkerneval(chnkr, ikern1,rho1, targets(:, in)) + coupled +chunkerkerneval(chnkr, ikern2, rho2, targets(:,in));
t2 = toc(start1);
...
[val,~] = chnk.flex2d.green(zk,[5;5],targets(:,in));        % Hankel part

zkimag = (1i)*zk;
[valK,~] = chnk.flex2d.green(zkimag,[5;5], targets(:,in));    % modified bessel K part
...
title("Absolute error on a large starfish", 'FontSize',16)
