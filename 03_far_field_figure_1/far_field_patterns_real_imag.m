...
coefs = [nu; 0];
ikern1 =  @(s,t) flex2d.suppkern(zk, s, t, 'supported plate starfish 3arms 1',coefs);           % build the desired kernel
...
ikern1 = @(s,t) flex2d.suppkern(zk, s, t, 'supported plate K1 eval starfish 3arms 1',coefs);                              % build the kernel of evaluation          
...
nexttile(2)
plot(thetas, real(ufar), 'Color', orange,'LineWidth',2)
hold on 
...
% legend('Clamped','Free','Supported','Dirichlet (Helmholtz)', 'Neumann (Helmholtz)', 'Location','southoutside')
...
exportgraphics(gcf,'far_field_figure_1.pdf','ContentType','vector')
