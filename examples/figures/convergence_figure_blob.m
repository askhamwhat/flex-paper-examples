...
for i = 1:numel(npts)
    ...
    cparams.maxchunklen = length;       % setting a chunk length helps when the
                                        % frequency is known'
    chnkr = chunkerfunc(@(t) blob(t), cparams);
    chnkr = chnkr.sort();
    npts(i) = chnkr.npt;
...
%    uerr = uerr ./  max(abs(true_sol));
    ...
    ikern1 =  @(s,t) flex2d.suppkern(zk, s, t, 'supported plate blob',coefs);           % build the desired kernel
    ikern2 =  @(s,t) flex2d.suppkern(zk, s, t, 'supported plate K21 blob',coefs);           % build the desired kernel
    ...
    ikern1 = @(s,t) flex2d.suppkern(zk, s, t, 'supported plate K1 eval blob',coefs);                              % build the kernel of evaluation          
