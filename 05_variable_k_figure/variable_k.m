...
minval = mean([min(abs(us{1}(:)))  min(abs(us{3}(:)))]);
maxval = mean([max(abs(us{1}(:)))  max(abs(us{3}(:)))]);

figure(1)
tiledlayout(1,3,"TileSpacing","compact")
nexttile
h = pcolor(Tx,Ty,abs(us{1}));
h.EdgeColor = 'none';
h.FaceColor = 'interp';
hold on
%colorbar
...
%%
saveas(gcf,'variable_k.fig','fig')
exportgraphics(gcf,'variable_k.pdf','ContentType','vector')
